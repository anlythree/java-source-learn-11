...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    static class Node<K,V> implements Map.Entry<K,V> {// 链表结构
        final int hash;
        final K key;
        V value;
        Node<K,V> next;
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                   boolean evict) {  //（hash值；key；value；如果是true不要更改现有值；如果为 false，则表处于创建模式）
        Node<K,V>[] tab; Node<K,V> p; int n, i; // tab：当前节点信息(链表结构) p：  n：当前HashMap元素数量
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
